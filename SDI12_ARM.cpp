...
Notes: finally, in function setState() it was necessary to setup _datapin as
INPUT before changing mode to OUTPUT, otherwise the pin wouldn't change state
working as output.

Data Line (depending on the board) can be set to:
* Leonardo: 0, 1, 2, 3, 7
...
#include <SDI12_ARM.h>						// 0.1 header file for this library
...
// 2.3 - Added MJB: parity fuction to replace the one specific for AVR from util/parity.h
